...
def import_and_process_1yr(y1, yN, y, thisYear, sdates_rx, hdates_rx, gddaccum_yp_list, gddharv_yp_list, skip_patches_for_isel_nan_lastyear, lastYear_active_patch_indices_list, incorrectly_daily, gddharv_in_h3, indir, incl_vegtypes_str_in, h1_ds_file, mxmats, get_gs_len_da, logger):
    ...
    h1_ds = utils.import_ds(h1_files, myVars=myVars, myVegtypes=utils.define_mgdcrop_list(), myVars_missing_ok=['GDDHARV'])
    if 'GDDHARV' not in h1_ds:
        ...
        try:
            ...
            if not h3_files:
                ...
                h3_files = glob.glob(pattern)
                if not h3_files:
                    error(logger, f"No files found matching pattern '*h3.{thisYear-1}-01-01*.nc(.base)'")
            h3_ds = utils.import_ds(h3_files, myVars=['GDDHARV'], myVegtypes=utils.define_mgdcrop_list())
...
if can_plot:
    ...
    def make_figures(first_land_use_year, last_land_use_year, land_use_file, run1_name, run2_name, logger, thisDir=None, gdd_maps_ds=None, gddharv_maps_ds=None, outdir_figs=None, linewidth=1.5):
        if not gdd_maps_ds:
            if not thisDir:
