...
def adjust_grainC(da_in, patches1d_itype_veg_str):
    ...
    drymatter_fractions = {
        ...
        "soybean": 0.91,
        "sugarcane": 1
        - 0.745,  # Irvine, Cane Sugar Handbook, 10th ed., 1977, P. 16. (see sugarcane.py)
        "wheat": 0.88,
    }
...
def convert_axis_time2gs_old(this_ds, myVars):
    ...
    if np.max(Nharvests_eachPatch) > Ngs + 1:
        raise ValueError(
            f"Expected at most {Ngs+1} harvests in any patch; found at least one patch with {np.max(Nharvests_eachPatch)}"
        )
    h = mxharvests
...
def fao_data_get(fao_all, element, y1, yN, fao_to_clm_dict, cropList_combined_clm):
    ...
    if len(fao_to_clm_dict) != len(cropList_combined_clm):
        raise RuntimeError(
            f"fao_to_clm_dict and are different lengths ({len(cropList_combined_clm)} vs {len(cropList_combined_clm)})"
        )
    if len(fao_this.columns) != len(cropList_combined_clm):
        raise RuntimeError(
            f"fao_this.columns and are different lengths ({len(fao_this.columns)} vs {len(cropList_combined_clm)})"
        )
...
def get_topN_ds(
    ...
    if earthstats_res not in earthstats:
        raise RuntimeError(
            f"What EarthStat resolution should be used to get country timeseries, if not {earthstats_res}?"
        )
            ...
            if country == "World":
                ...
                if len(country_id) != 1:
                    if len(country_id) == 0:
                        continue
                    raise RuntimeError(
                        f"Expected 0 or 1 match of {country} in countries_key; got {len(country_id)}"
                    )
    ...
    for c, thisCrop in enumerate(cropList_combined_clm[:-1]):
        ...
        for pft_str in yield_gd.ivt_str.values:
            ...
            if map_prod_thisCrop_clm.shape != map_yield_thisCrop_clm.shape:
                raise RuntimeError(
                    f"Error getting production map {map_yield_thisCrop_clm.dims}: expected shape {map_yield_thisCrop_clm.shape} but got {map_prod_thisCrop_clm.shape}"
                )
...
def get_ts_prod_clm_yc_da2(case_ds, lu_ds, yieldVar, cropList_combined_clm, quiet=False):
    ...
    if len(non_patch_dims) != 1:
        raise RuntimeError(
            f"Expected one non-patch dimension of case_ds['{yieldVar}']; found {len(non_patch_dims)}: {non_patch_dims}"
        )
    ...
    if time_dim_in != "time":
        if not quiet:
            print(
                f"WARNING: Using calendar years from LU data with yield data of time dimension {time_dim_in}."
            )
...
def get_yield(ds, **kwargs):
    ...
    for key, selection in kwargs.items():
        if key == "out_var" and selection != "YIELD":
            raise RuntimeError(
                "get_yield() may only be called with out_var='YIELD'. Did you mean to call zero_immatures() instead?"
            )
...
def print_gs_table(ds):
    ...
    for v in ["gs", "SDATES", "GDDHARV", "HDATES", "HARVEST_REASON"]:
        ...
        data[v] = ds[v].values
    print(
        f"Lon {ds.patches1d_lon.values} lat {ds.patches1d_lat.values}, {ds.patches1d_itype_veg_str.values}"
    )
...
def print_onepatch_wrongNgs(
    ...
    print(
        f"patch {p}: {this_ds_orig.patches1d_itype_veg_str.values[p]}, lon {this_ds_orig.patches1d_lon.values[p]} lat {this_ds_orig.patches1d_lat.values[p]}"
    )
    ...
    if "pandas" in sys.modules:
        ...
        print_pandas_ymp(
            'After "In years with sowing that are followed by inactive years, check whether the last sowing was harvested before the patch was deactivated. If not, pretend the LAST no-harvest is meaningful."',
            ["sdate", "hdate"],
            (
        ...
        print_pandas_pg(
            'After "Ignore any harvests that were planted in the final year, because some cells will have incomplete growing seasons for the final year"',
            ["sdate", "hdate"],
        ...
        print_nopandas(
            np.transpose(sdates_pym3, (1, 2, 0))[:, :, p],
            np.transpose(hdates_pym3, (1, 2, 0))[:, :, p],
            'After "In years with sowing that are followed by inactive years, check whether the last sowing was harvested before the patch was deactivated. If not, pretend the LAST [easier to implement!] no-harvest is meaningful."',
        )
...
def round_lonlats_to_match_da(ds_a, varname_a, tolerance, ds_b=None, varname_b=None):
    ...
    if max_diff > tolerance:
        ...
        if varname_b:
            print(
                f"Maximum disagreement between {varname_a} and {varname_b} ({max_diff}) is larger than default tolerance of {tolerance}. Increasing tolerance to {new_tolerance}."
            )
        else:
            print(
                f"Maximum disagreement in {varname_a} ({max_diff}) is larger than default tolerance of {tolerance}. Increasing tolerance to {new_tolerance}."
            )
    ...
    def replace_vals(ds, new_da, varname):
        if len(ds[varname]) != len(new_da):
            raise RuntimeError(
                f"Not allowing you to change length of {varname} from {len(ds[varname])} to {len(new_da)}"
            )
        if varname in ds.coords:
            ...
            raise TypeError(
                f"{varname} is not a coordinate or data variable; update round_lonlats_to_match_da()->replace_vals() to handle it"
            )
        return ds
...
def time_units_and_trim(ds, y1, yN, dt_type):
    ...
    if not isinstance(time0, cftime.datetime):
        ...
        print(
            f"Converting integer time to {dt_type}, assuming values are years and each time step is Jan. 1."
        )
        ...
        if min_viable_hui == "isimip3" or min_viable_hui == "ggcmi3":
            ...
            for veg_str in np.unique(ds.patches1d_itype_veg_str.values):
                ...
                if patch_index == 0:
                    ...
                    raise RuntimeError(
                        f"Temporarily rearrange min_viable_hui_touse so that patch dimension is first (0) or last ({ds[huifrac_var].ndim - 1}), instead of {patch_index}."
                    )
        elif isinstance(min_viable_hui, str):
            raise RuntimeError(
                f"min_viable_hui {min_viable_hui} not recognized. Accepted strings are ggcmi3 or isimip3"
            )
        else:
            min_viable_hui_touse = min_viable_hui
        if np.any(huifrac < min_viable_hui_touse):
            print(
                f"Setting {out_var} to zero where minimum viable HUI ({min_viable_hui}) wasn't reached"
            )
