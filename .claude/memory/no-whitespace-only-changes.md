...
**Why:** Whitespace-only edits inflate the diff, land in `git blame` against the user's name, and in this repo manufacture merge conflicts with the `ctsm5.4.028_nvp` branch for no benefit. They raised it after two tasks in a row picked up this churn.
