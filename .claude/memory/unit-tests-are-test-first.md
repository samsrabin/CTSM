...
**Retrofitting coverage to code that already landed** (added 2026-08-27, NVP Task 5g): there is no red-first track at all, so the whole task is green-throughout and the writer expects green. The mutation duty then splits — the writer still mutates every assertion it lands, **and** the reviewer independently mutates too, choosing its own mutations rather than re-running the writer's. Sam's reason: a test that detects only the mutation its author had in mind reads as coverage without being coverage. A mutation counts only if it is two-sided (red on the new case, green on its stock mirror), and reverting the line to its pre-change form beats inventing a mutation.
