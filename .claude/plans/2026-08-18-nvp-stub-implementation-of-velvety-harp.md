...
| 10.4e | Layerless snow (`h2osno_no_layers > 0, snl == 0`) over moss | Unit | Reachability at ALP2 is unknown; if the site never produces it, this needs a unit test rather than a suite entry. And anyway a suite entry wouldn't be straightforward to check. Thus, require unit tests. **Need to determine where these unit tests will come in.** |
