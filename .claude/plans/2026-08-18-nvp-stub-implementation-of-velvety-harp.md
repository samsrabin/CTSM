...
- [ ] **Step 4 — whole-pack disappearance.** `zwice`/`zwliq` sum over `col%get_jtop_snow(c) : col%get_jbot_snow(c)`. Liquid goes to moss where `col%nvp_is_present(c)`, else to the stock target `h2osoi_liq(c,1)` for soil/crop/urban.
