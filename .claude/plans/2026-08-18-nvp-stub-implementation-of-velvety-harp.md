...
- [ ] **Step 3 — `swe_old` ([fix]).** Their branch left this bare loop including the moss slot while `AerosolMod` excludes it — an inconsistency feeding SNICAR/aerosol scaling (spec §5). **There are two loops, not one**, both `j`-inner so both take real bounds: `:500` zeroes above the pack (`-nlevsno+1, snl(c)` → `-nlevsno+1, col%get_jtop_snow(c)-1`) and `:503` fills the pack (`snl(c)+1, 0` → `col%get_jtop_snow(c), col%get_jbot_snow(c)`).
...
- [ ] **Step 7 — remove the last stopgap.** Delete the entry `endrun` 5a placed in `DivideSnowLayers`, and the `use_nvp` import at `SnowHydrologyMod:26` that exists solely for it. After this `use_nvp=.true.` runs to completion instead of aborting — it is **not** correct until Task 14, per Step 0.
