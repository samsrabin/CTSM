...
The rows are of three kinds, because §10's items are not all the same shape:
...
| 10.5 | Golden zero-thickness invariant | Suite entry + manual | **Rescoped 2026-08-27, and the "exactly or to roundoff" ambiguity in §10.5 is now closed: to roundoff.** Spec §3 keeps the moss row in the matrix, because deleting it would make the snow↔soil coupling span 3 rows and break `nband = 5`. So the band solve carries one more row than the `use_nvp = .false.` run, `dgbsv` takes a different pivot sequence, and the two differ in the last bits from the first solve — and the snow-bottom row's own coefficient is a different number, equal to stock's only after elimination. A `SystemTestsCompareTwo` test type in the `LWISO`/`LCISO` mould therefore cannot pass, since that framework has no tolerance anywhere in its cprnc path. What the suite entry asserts instead is that a `dz_nvp = 0` run **completes with every balance check armed**, which exercises the skip paths §10.5 exists for; the routing decisions themselves are pinned directly by unit tests. A `cprnc` between the NVP-on and NVP-off runs stays available as a **diagnostic**, to catch gross divergence rather than as a PASS/FAIL gate. |
...
| 10.8a | Unit tests both ways | Standing | The unit-test run in every task's build check, plus the per-task authoring rule in Global Constraints, plus the Task 16 audit. |
| 10.8b | aux_clm lake / glacier / urban b4b | Standing | Those landunits always have `jbot_sno = 0`, so this is the **Suites** bullet again — but Task 18 must confirm the suite actually contains lake, glacier and urban coverage rather than assuming it. |
