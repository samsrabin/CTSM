...
- **Neither this plan file nor `MERGE_NOTES.md` survives the upstream merge, so anything that has to persist goes in the code.** Both exist to run and land this branch and are deleted with it; a reader of merged CTSM will have neither. So a constraint a later maintainer must not violate, a reason a guard is shaped the way it is, or a divergence that will look like a mistake without its justification belongs in a comment at the site — the plan file and `MERGE_NOTES.md` hold only what is needed *before* the merge: sequencing, review state, and the merge bookkeeping Task 18 consumes. When a decision produces both kinds, write the durable half as a comment first, then reference it. **Which of a task's divergences need a comment at the site is settled in that task's Step 0**, against the code it touches — the existing MERGE_NOTES instructions in the task steps were not swept for this and are not evidence that a divergence needs no comment.
- Fortran style: match surrounding code (2-space indent, `_r8` literals, `associate` blocks, `SHR_ASSERT_ALL_FL` for bounds).
- **When unit tests get written, and how they are reviewed.** The rules are in the **`writing-tests-before-the-implementer`** skill — invoke it before writing, committing or reviewing any `.pf` file. It carries the two tracks (red-first and green-throughout), the standalone never-amended test commit, the must-build exception, the mutation duty, the anti-retrofit diff check and its `:(top)` trap, and what to do when a test has to change. Specific to this branch:
  - **Each task adds the unit tests for the code it lands**, at the earliest task where the test can actually be written, with the writer dispatched ahead of the implementer at Execution Process step 3a. A test that genuinely needs code from a later task belongs to that later task — that is a dependency, not a deferral.
  - **Why this branch does not batch them at the end:** the defects these tests catch are the ones the suites structurally cannot see. `BalanceCheckMod.F90:764` gates the whole snow balance on `snl < 0` and forces `errh2osno = 0` otherwise, and `BalanceCheck` runs after `HydrologyNoDrainage` — so on the timestep the last snow layer dissolves into the moss, the check is skipped entirely. Both NVP interface-recursion guards passed 60/60 binaries while mutated. And for Tasks 6-10 there is no `use_nvp = .true.` system-test signal at all, because nothing completes a run until Task 11 at the earliest, so "wait for a suite failure to tell us we need a unit test" has nothing to trigger on across the hardest stretch of index rewriting.
- **Writing unit tests and fixtures.** The build, the fixtures CTSM already provides and the pFUnit authoring limits are in **`ctsm-unit-tests`**; choosing a test's values, naming it and writing its opening comment are in **`designing-unit-test-cases`**. Invoke both before writing or editing any `.pf` file. Between them they carry the `water_type` factory's call order, hand-allocating a type that has no factory and why the NaN prefill matters, the `CESM_DEBUG` flag set, the stale-driver trap and what counts as evidence that a test ran, markers and degenerate fixtures, the topline-summary rules, named constants, and array-bound comments. Specific to this work:
  - **`col%jbot_sno` is a plain array** — assign it directly in the `.pf` after the subgrid setup, as `test_initSnowLayers_overfillPack_unevenSplit_nvp` does. No `unittestSubgridMod` change is ever needed for an NVP test.
  - **`dzmin`/`dzmax_u`/`dzmax_l` are allocated inside `InitSnowLayers`**, and `SnowHydrologySetControlForTesting()` sets only scalar namelist values, so a test that reads them must call `InitSnowLayers` first (a zero snow depth serves as a bare allocator) and call `SnowHydrologyClean()` in `tearDown`.
  - **The snow-layer test names** are `overfillPack_unevenSplit`, `partialPack_evenSplit`, `singleLayer` and `emptyPack`, each suffixed `_stock` and `_nvp`. **`overfillPack` stays in the first of those** (user, 2026-08-27): the pack state is what makes the layer-count cap rather than the depth the binding constraint, which is the entire NVP point, and the branch name alone loses it.
  - **Where the named constants collapse a `_stock`/`_nvp` pair into one line:** on an NVP column `jtop = snl_before`, on a stock column `jtop = snl_before + 1`. That is the whole reindex in miniature.
...
- **Adding and changing system tests, and running them.** The rules are in **`ctsm-system-tests`** and `wallclock.md` beside it — invoke it before touching `testlist_clm.xml`, a testmod or `ExpectedTestFails.xml`, and before saying anything about a suite result. It carries that the suites are the user's to launch and that you never report or characterize a result you were not given, choosing a test type and what each one actually assesses, registering an entry, categories, `ExpectedTestFails` phases and the baseline-instrument prohibition, the four ways to get a testmod silently wrong, machine and compiler coverage, deriving a wallclock, baselines, and when a commit stops being amendable. Specific to this work:
  - **Every task states its expectations in a "Testing changes and expectations" block** at the bottom of its section.
  - **Our categories are `nvp` (anything with NVP on), `bigleaf_nvp` (NVP on, FATES off) and `fates_nvp` (NVP and FATES on).** Every NVP entry also carries `aux_clm`, and `fates` where FATES is on.
  - **The NVP test set is required to contain all of the following** (the user, 2026-08-26). This is a floor, not a target: site-level tests at ALP2 with NVP on, **plus at least two global tests, one with FATES on and one with FATES off**, and at least one exact-restart test (`ERS`, or `ERP` where a PE-layout change adds something). No `ER*` test may exactly duplicate an `SMS` test. The global pair is not an extra — some requirements can only ever be met there, because ALP2 carries no lake, glacier, urban or wetland landunit at all (its fsurdat is `PCT_LAKE = 0`, `PCT_GLACIER = 0`, `PCT_URBAN = 0, 0, 0`, `PCT_WETLAND = 0`), so anything about those landunits under `use_nvp = .true.` is invisible to every site test.
  ...
  - **Testmod composition order here is feature-then-site**, applied consistently, since later testmods win.
- **Commit messages.** The conventions are in your global `~/.claude/CLAUDE.md` and bind every commit made from here on, including the ones subagents make. Specific to this branch:
  - **They reach forward only. No existing commit message is ever rewritten to match them** — not the ones already on this branch, not a cherry-picked message, not one a review finding touches.
  - **The model is the "In brief" paragraph at the top of each task** — plain language, says what the change does and why it is wanted, names the routines and the spec section, and stops. Match its register and its level of technical depth; the message may run longer than the paragraph, and usually will, since it covers only what this commit landed but covers all of it. The `git commit -am "..."` string in a task's step text is the subject line, not the message; where a task has no "In brief" paragraph (5a-5c), write in that same register from the task's step text.
  - **Here, a number counts as sourced** if it carries a spec §, a `theirs :NNNN` reference, a `file:line`, or the run that produced it.
  - **Where the explanation already lives at the site, the message says only that the change happened.** That is the common case in this work: every guard added by the reindex tasks carries a constraint comment beside it, so the message says the routine was reindexed, not why the guard is shaped as it is. Every deliberate divergence from `ctsm5.4.028_nvp` or from CTSM convention is still named in a clause, since `MERGE_NOTES.md` carries only the merge bookkeeping and goes away at the merge.
  - **Say plainly when no system test has been run against the commit** — the suites are the user's to launch, so that is the normal state at a hand-off rather than an omission.
  - **One thing reads like development history and is not:** a step-3a test commit's message says which of its cases fail at that commit and that they fail by design, because that is the commit's state and a reader who checks it out needs it.
...
3a. **Fresh test-writing subagent, dispatched before the implementer.** It receives the same materials the implementer will get — the task text as amended in Step 0, the spec path, the dedicated-checkout path, the Global Constraints above — and nothing about how the implementation is to be shaped. **Its dispatch names `writing-tests-before-the-implementer`, `ctsm-unit-tests` and `designing-unit-test-cases` and says to invoke all three before writing anything** — between them they carry what it runs, what it labels, what evidence it owes, and how it commits. If the task lands nothing a unit test can reach, it says so and says why.
4. **Fresh implementer subagent per task.** The subagent receives: this plan's task text as amended in Step 0 (only its own task), the spec path, the harvest-worktree path, the dedicated-checkout path, and the Global Constraints above. **Its dispatch states that it must not edit any `.pf` file** and names `writing-tests-before-the-implementer` for why: the tests are already committed, some of them failing, and making them pass without touching them is the task.
5. Each task ends with: verification passes (below) → **one commit** for the implementation, sitting on top of step 3a's test commit (and on top of a separate commit for any test correction — a test that had to change is a finding, and never rides along in the implementation commit).
   ...
   - **How the message itself is written is set by the commit-message constraints in Global Constraints and in your global `~/.claude/CLAUDE.md`.** The `git commit -am "..."` string in the step text is only the subject line.
6. After the commit, run **two-stage review** (superpowers:requesting-code-review pattern): first a **spec-compliance** reviewer (does the diff implement this task's requirements and only them? cite spec §), then a **code-quality** reviewer (correctness, style, conservation hazards). Reviewers are fresh subagents that see the task text, the Global Constraints above, and the diff, which for these purposes is the **union of the step-3a test commit and the implementation commit** (`writing-tests-before-the-implementer` says why). The spec-compliance reviewer additionally confirms the test files were not touched after they were run.
7. Issues found → fix → **amend the task's implementation commit** (`git commit --amend --no-edit`) → re-run the failed review stage. **Never amend a test commit** — a finding against a test becomes its own new commit, per Global Constraints.
...
- **Build check**: from the dedicated checkout, `cd test-bld && qcmd -- ./case.build` (a dedicated case at the top level of the checkout). Expected: build completes with no errors. `test-bld/` is write-protected — run the build from it, never edit anything under it.
  - **When the task adds a new `.F90`**, the build cache will not notice it: CIME derives `Srcfiles` from `Filepath`, and adding a file to an existing source directory does not touch `Filepath`, so the build fails with `error #7002` (`ctsm-system-tests` has the mechanism). Force the refresh first — `touch /glade/derecho/scratch/samrabin/test-bld/bld/intel/mpich/debug/nothreads/clm/obj/Filepath` — or `./case.build --clean lnd`. Confirmed empirically in Task 1.
- **Unit tests** (required for any commit that touches Fortran): from the checkout's `src/` directory, `qcmd -- ../cime/scripts/fortran_unit_testing/run_tests.py --build-dir unit_tests.temp`. Expected: all tests pass.
  - **The step-3a test commit is the one exemption**, and only from "all tests pass" — see `writing-tests-before-the-implementer`. It must still **build**, and every test that existed before it must still pass. Every later commit in the task is held to the full gate.
  - **When the task adds a `.pf` file**, `rm -rf src/unit_tests.temp` first, then confirm the new tests ran by reading the per-directory executable's own `(N tests)` line — **not** ctest's `out of N`, which counts executables and so cannot move when a file is added to an existing test directory. `ctsm-unit-tests` has the trap's exact boundary and the single stale object to delete if a full rebuild is too expensive. Confirmed empirically in Task 5d, where the first run silently re-ran the pre-task cases.
- **Test-diff check** (every task that has a step-3a test commit): from the **repository root**, `git diff <test-commit>..HEAD -- ':(top)*.pf'` must come back empty. The `:(top)` anchor is required — run from inside a test directory the same command reports on that directory alone and comes back clean about a sibling that changed. Check the implementer's whole diff for test-side files the glob does not match (`src/unit_test_shr/`, a test directory's `CMakeLists.txt`), since a red test can be turned green by editing any of them. A non-empty result means either a test was retrofitted to the implementation, or Step 0 misread the requirement. Both are findings; the second gets its own commit and a correction to the task text, and neither may pass silently.
...
**The harness facts this task established** — the water-type factory ordering, hand-allocating `temperature_type`/`aerosol_type`, the `nlevsno` and `dzmin`/`dzmax` setup traps, and the pFUnit authoring limits — **are now carried by the `ctsm-unit-tests` skill**, except the `dzmin`/`dzmax` trap, which stayed in Global Constraints as NVP-specific.
...
**Names follow the two tests already in that file**, which were renamed on 2026-08-27 from `overfillPack_{stock,nvp}` to `overfillPack_unevenSplit_{stock,nvp}` precisely so this task's additions would have something to be parallel to — see `designing-unit-test-cases` for the rule and Global Constraints for the name set. Each new case is a `_stock` / `_nvp` pair named for the branch it takes. **Leave `test_initSnowLayers_depth0_snl` and `test_initSnowLayers_depth1_snl` alone**: both are stock `ctsm5.4.028` tests, so renaming them to match would be divergence from the merge target for no gain, and `depth0_snl`'s single weak assertion is a reason to add `emptyPack_stock` beside it rather than to edit it.
...
- [ ] **Step 2 — the empty-pack branch on an NVP column (`:3150-3159`).** `test_initSnowLayers_emptyPack_{nvp,stock}`. The highest-value hole. This branch writes its own blanket slices, and none of them has ever run with `jbot == -1`, because every NVP column in the file overfills. Wanted: `jbot_sno = -1`, moss geometry laid down, `snow_depth < dzmin(1)`.
...
- [ ] **Step 3 — the even-split branch (`:3196-3197`).** `test_initSnowLayers_partialPack_evenSplit_{stock,nvp}`. Two `jbot`-indexed assignments with zero coverage on any column, so a substitution botched only in this branch is invisible today. Both overfill fixtures land in the `else` at `:3199-3200`, and must — see `overfill_factor`'s comment. Wanted: a depth that takes the even split **and** leaves the NVP layer count below the cap, which also gives the file its first NVP case where depth rather than the moss slot sets the count. At `nlevsno = 5` the window for `-snl = 3` with an even split is `sum(dzmax_u(1:2)) < snow_depth <= dzmax_u(1) + 2*dzmax_u(2)`, i.e. `(0.07, 0.12]`; its midpoint lays down `0.02 / 0.0375 / 0.0375` at slots `-2..0` stock and `-3..-1` with moss at 0. Derive the depth from `dzmax_u` the way `overfill_depth` does — do not name a literal.

- [ ] **Step 4 — the single-layer assignment (`:3169`).** `test_initSnowLayers_singleLayer_{stock,nvp}`. One line, `dz(c,jbot) = snow_depth(c)`, executed by no test at all. On an NVP column a slip to `dz(c,0)` writes the snow depth straight into the moss thickness. Any depth in `[dzmin(1), dzmax_l(1)]`; cheapest of the three, and the pair is small.
...
- **Folded in 2026-08-27 from the per-task run/abort analysis — assigned to no task before now:**
  ...
  - `src/biogeophys/SnowSnicarMod.F90` — **the plan never opened this file**; it appeared nowhere in the plan, the spec, or MERGE_NOTES until 2026-08-28, when the merge comparison established why: their branch's 114-line change to it is SNICAR "Approach B", the full snow->moss->soil radiative transfer that spec §8 defers outright, so its absence here is the deferral working rather than a gap. What is *not* covered by that deferral is the snow-index work below, which their branch does not touch either — their diff stops at their :1230, well above `SnowAge_grain`. Take the two sites that belong to this task's domain: `SnowAge_grain`'s `snl_top`/`snl_btm` window and `cdz` (`:1576-1579`), where `cdz(snl_top:snl_btm)` at `:1579` reaches the moss slot because `snl_btm = 0` (`:1576`), making `cdz(0) = frac_sno*dz(c,0)` a **fifth** zero-thickness divide-by-zero at `:1604` and `:1607` — note this half is wrong on *their* branch too, since `snl_btm = 0` is the moss slot under either `snl` convention; only the `snl_top = snl(c_idx)+1` half at `:1577` is ours alone — put it on the "fix this one first" list beside `clm_driver.F90:1637` — and the hardcoded `snw_rds(c_idx,0) = snw_rds_min` at `:1721`. Also `src/biogeophys/WaterDiagnosticBulkType.F90:1283` (`ResetBulk` writing `snw_rds_col(column,0)`, called from `SnowHydrologyMod.F90:855` for a freshly created pack — it writes the moss slot instead of the new snow layer). The `SNICAR_RT` slot mapping at `:690-691` is Task 12's, not this task's; cross-reference so the two do not both edit the file blind. **Check the ordering hazard:** once this task reindexes `AerosolMasses`, its `else` branch sets `snw_rds(c,0) = 0`, and SNICAR `endrun`s on an out-of-table grain radius at `:712-721`. That is probably masked by `SnowAge_grain`'s `snw_rds_min` clamp running first, but it depends on `filter_snowc` membership matching between the two calls — verify rather than assume.
...
- [ ] **Step 0a — write and commit this task's unit tests, before any of its implementation exists.** Dispatch the test-writing subagent (Execution Process step 3a). It writes the pFUnit coverage for what this task will land, **runs it**, and labels each test red-first or green-throughout: the green ones carry mutation evidence into the hand-off, the red ones carry the fact that they failed here. Standalone commit, never amended afterwards. The implementer that follows may not touch these files, and they must pass **unchanged** by the end of the task. If this task genuinely lands nothing a unit test can reach, say so explicitly and say why; silence is not a considered "none". **This task owes one test specifically**, recorded in the §10 coverage table as an uncovered §10.5 skip path: the zero-`dz` percolation routing. Its design constraint is that the deposit into `h2osoi_liq(c,0)` and the removal of that water from `qflx_rain_plus_snomelt` are complementary halves — **the test must fail if either is done without the other**, because one that checks only the deposit passes on a double-count. The `dz_nvp = 0` case cannot see this at all: neither half runs there, so unit tests are the only coverage it will ever have.
...
- [ ] **Step 0a — write and commit this task's unit tests, before any of its implementation exists.** Dispatch the test-writing subagent (Execution Process step 3a). It writes the pFUnit coverage for what this task will land, **runs it**, and labels each test red-first or green-throughout: the green ones carry mutation evidence into the hand-off, the red ones carry the fact that they failed here. Standalone commit, never amended afterwards. The implementer that follows may not touch these files, and they must pass **unchanged** by the end of the task. If this task genuinely lands nothing a unit test can reach, say so explicitly and say why; silence is not a considered "none". **This task also owes the layerless-snow test recorded in the §10 coverage table (10.4e):** with `snl == 0` and `h2osno_no_layers > 0`, Step 4's heat capacity must land on `cv(c,0)` where the moss is present and stay on `cv(c,1)` where the slot is empty or the column is stock.
...
- [ ] **Step 5b — ALP2 testmods.** Four levels, feature-then-site composition order, no testmod overriding another's `user_nl_clm` line (that is not reliable):
  ...
  - `clm/NvpMossFull` — `include_user_mods: ../Nvp`; `dz_nvp` as above with `frac_nvp = 1`. Complete cover is an endpoint of the fraction rule, and the one place `frac_soil` goes to zero.
  - `clm/NvpMossNoCover` — `include_user_mods: ../Nvp`; `dz_nvp` as above with `frac_nvp = 0`. **Create this only if Task 8 Step 0 decides that state stays legal.** `controlMod.F90:645-677` admits it today, but it is the `1/frac_nvp` division hazard: `nvp_is_present(c)` is true, every `[physics]` gate fires, the moss row carries real heat capacity, and any weight of the form `1/frac_nvp` is `1/0`. If Step 0 instead adds a validity check rejecting it, the right test is a `build-namelist_test.pl` failtest, not a system test — say which was done.
- [ ] **Step 5c — testlist entries and the three suites.** Every entry carries `aux_clm`, the applicable `nvp` categories, and `fates` where FATES is on. The set below is the floor Global Constraints require; derive each wallclock from an existing entry at the same grid and duration and say which.
  ...
  - **Global — a factorial over FATES, thickness and cover.** Global tests are not optional and are wanted in quantity: ALP2 carries no lake, glacier, urban or wetland landunit, so global entries are the only thing that satisfies §10 coverage row 10.8b, and a global grid samples every climate and snow state at once. **`SMS_Ld5_D` is enough for these** — a five-day global run has snow somewhere regardless of start date, which is exactly what a single arctic point cannot give you at that length. Pick the start date so the northern hemisphere is in winter, and say so.

    The factorial is **six cells, not twelve**, because two combinations are namelist errors by construction: `controlMod.F90:658` rejects `dz_nvp == 0 .and. frac_nvp > 0`, so zero thickness admits only zero cover. Per FATES setting — off and on — the valid cells are `clm/Nvp` (zero thickness, zero cover), `clm/NvpMoss03` (thickness, partial cover) and `clm/NvpMossFull` (thickness, complete cover). A seventh and eighth cell, `clm/NvpMossNoCover` with each FATES setting, exist only if Task 8 Step 0 keeps `dz_nvp > 0, frac_nvp = 0` legal.

    Note what each FATES half is for. The FATES-**off** cells are the broad test that every guard reduces to stock across many column types and climates, and the zero-thickness one is what satisfies row 10.8b. The FATES-**on** cells exist for the two FATES risk areas — `hs_nvp` accumulating correctly for any patch structure (Task 11 Step 3) and radiation through `sabg_lyr` (Task 12) — **and neither fires at zero thickness**, where `frac_nvp_eff` is identically 0, the 4-way partition collapses to the stock 3-way and `sabg_nvp` is forced to zero. So the FATES-on zero-thickness cell is a cheap smoke test, and the two with moss are the ones that actually cover the risk.
...
- [ ] **Step 5d — `ExpectedTestFails.xml` entries, and the removals they imply.** Every NVP-on entry above fails at this task and must land with an entry naming the phase, keyed on the full test name with no testid suffix. Per §10 coverage, the schedule is by **configuration**, not by grid scale — every entry with `dz_nvp = 0`, at ALP2 and global alike, cannot complete a run before **Task 11**; every entry with `dz_nvp > 0`, whatever its cover, not before **Task 14**. Write the removals into those tasks as explicit steps now, in the same commit — an entry with no scheduled removal is how expected fails become permanent.
- [ ] **Step 6: Run build check.**
...
- **Expected fails.** Every NVP-on entry, each naming the phase it fails in. Every `dz_nvp = 0` entry — the ALP2 pair and the two zero-thickness globals — retires at Task 11 (Step 4b). Every `dz_nvp > 0` entry, at ALP2 and global alike and whatever its cover, retires at Task 14 (Step 3b).
...
- [ ] **Step 0a — write and commit this task's unit tests, before any of its implementation exists.** Dispatch the test-writing subagent (Execution Process step 3a). It writes the pFUnit coverage for what this task will land, **runs it**, and labels each test red-first or green-throughout: the green ones carry mutation evidence into the hand-off, the red ones carry the fact that they failed here. Standalone commit, never amended afterwards. The implementer that follows may not touch these files, and they must pass **unchanged** by the end of the task. If this task genuinely lands nothing a unit test can reach, say so explicitly and say why; silence is not a considered "none". **This task owes two tests specifically**, both recorded in the §10 coverage table as uncovered §10.5 skip paths: the **§3 degenerate row** — the flux-continuity equation the moss row carries when `nvp_is_empty`, including its collapse to `T_0 - T_1 = 0` at `snl == 0` — and the **§4b fraction rule**, that `NVPEffectiveFractions` returns `frac_nvp_eff = 0` whenever `dz(c,0) = 0` and that the 4-way partition sums to 1 across admissible fractions. Both are first writable here, because neither the row nor the routine exists before this task. **The fraction-rule test is this plan's one known instance of the must-build exception** (Global Constraints, and `writing-tests-before-the-implementer` for the rule): `NVPEffectiveFractions` does not exist yet, so a test calling it will not build at Step 0a, and it is a broken build rather than a red test. It commits instead at the earliest step that makes it compile — which requires Step 0 to first settle *which* step lands the routine, since this task's "In brief" says the task lands it while its Interfaces block still attributes it to Task 3 and no numbered step claims it. The degenerate-row and `cv(c,0)` tests have no such problem and go in at Step 0a with the rest. **A third test, from §10 coverage row 10.4e:** that `cv(c,0)` is actually consumed once the moss row is in the solve. Between Task 7 and this task the layerless-snow heat capacity is assigned to a row the system does not contain, because `jtop(c) = snl(c) = 0` on a snow-free NVP column — it is silently absent from the solve, and no armed check sees it.
...
- [ ] **Step 0a — write and commit this task's unit tests, before any of its implementation exists.** Dispatch the test-writing subagent (Execution Process step 3a). It writes the pFUnit coverage for what this task will land, **runs it**, and labels each test red-first or green-throughout: the green ones carry mutation evidence into the hand-off, the red ones carry the fact that they failed here. Standalone commit, never amended afterwards. The implementer that follows may not touch these files, and they must pass **unchanged** by the end of the task. If this task genuinely lands nothing a unit test can reach, say so explicitly and say why; silence is not a considered "none".
...
- [ ] **Step 3 [harvest+fix]:** `ComputeGroundHeatFluxAndDeriv`: `lwrad_emit_nvp = emg*sb*t_soisno(c,0)**4`, **and two statements above it fix `lwrad_emit_snow(c)` at `:1670`**, which reads `t_soisno(c,snl(c)+1)` and so takes its emission temperature from the second snow layer — the moss slot when there is only one snow layer. Found 2026-08-28 by the merge comparison, previously assigned to no task; same class as `lyr_top` (Task 8) and in the same routine, but independent of the matrix rows, so it belongs here beside the term it partners. Their branch leaves it stock, correctly for their convention. MERGE_NOTES row filed; `hs_nvp` accumulation over patches — **[fix]** the accumulation must use the same patch gate as the flux definitions so atmosphere-seen flux == moss-lost flux for any patch structure (audit's non-veg-patch inconsistency).
...
- [ ] **Step 4b — retire the `ExpectedTestFails.xml` entries this task earns.** Task 7 Step 5d added entries for every NVP-on test because none could complete a run. This task is where every `dz_nvp = 0` entry starts passing — the ALP2 zero-thickness pair and both zero-thickness globals — because Step 4's `errsoi` window is the last thing keeping them from closing. **Remove those entries in this task's commit**, and if any of them still fails, say so in the hand-off rather than restoring the entry: a still-failing test after this task means a fix is missing, not that the expectation was right. Every `dz_nvp > 0` entry stays; those are Task 14's to retire.
...
- **Suites.** `aux_clm` and `fates` pass, every pre-existing test bit-for-bit against the previous baseline. **Every `dz_nvp = 0` entry starts passing here** — the ALP2 zero-thickness pair and both zero-thickness globals — since Step 4's `errsoi` window is the last thing keeping them from closing. Every `dz_nvp > 0` entry remains an expected fail until Task 14.
...
- **Tests added or changed.** No new entries. Step 4b removes the `ExpectedTestFails.xml` entries for every `dz_nvp = 0` test.
- **Expected fails.** Zero-thickness and global entries retired. Every `dz_nvp > 0` entry remains, and those are Task 14's to retire. If a retired test still fails, that is a missing fix — report it rather than restoring the entry.
...
- **Suites.** `aux_clm` and `fates` pass, every pre-existing test bit-for-bit against the previous baseline. Every `dz_nvp = 0` entry, passing since Task 11, compares b4b against the most recent baseline containing it. Every `dz_nvp > 0` entry remains an expected fail until Task 14.
- **Answer changes.** **None in any test that is currently passing** — and that is a falsifiable claim, not a hope. Spec §4f forces transmissivity to 1 wherever `dz(c,0) = 0`, so a zero-thickness moss absorbs nothing and the partition reduces to stock deposition. Every passing NVP entry runs `clm/Nvp`, which is the `dz_nvp = 0` case. **So if any `dz_nvp = 0` entry moves at this task, the forced-transmissivity rule is broken.** Real answer changes are confined to the partial-cover configurations, which no passing test exercises yet.
...
- [ ] **Step 3b — retire the `ExpectedTestFails.xml` entries for every `dz_nvp > 0` test.** The ALP2 `clm/NvpMoss03`/`clm/NvpMoss07` pair and the global moss cells added at Task 7 Step 5d have been failing since then because `qflx_nvp_to_snow_col` raised `h2osno_total` with no matching term in `snow_sources`. Step 3 adds that term, which is the last thing keeping them from closing. **Remove them all in this task's commit.** With that, no NVP test carries an expected fail; if one still does, it is a gap, not an expectation.
...
- **Suites.** `aux_clm` and `fates` pass, every pre-existing test bit-for-bit against the previous baseline. **Every `dz_nvp > 0` entry starts passing here** — the two ALP2 partial-cover runs and the global moss cells, both FATES settings — once Step 3 adds `qflx_nvp_to_snow_col` to `snow_sources`. With that, no NVP test carries an expected fail.
- **Answer changes.** None in pre-existing tests, and none in the `dz_nvp = 0` entries — but the risk to watch is Step 1. Restructuring the `TotalWaterAndHeatMod` sums must add the moss terms **only inside `if (col%nvp_layer_exists(c))`**: adding zero-valued terms unconditionally changes the summation order on stock columns and moves the last bit. Answers for every `dz_nvp > 0` entry are established for the first time here, so there is no prior baseline for them to move against.
- **Tests added or changed.** No new entries. Step 3b removes the `ExpectedTestFails.xml` entries for every `dz_nvp > 0` test. Unit tests: the `Balance_test` NVP coverage and the non-soil-landunit test, both from §10 coverage rows 10.8a and 10.8b.
- **Expected fails.** Every `dz_nvp > 0` entry retired. **None should remain on any NVP test after this task**; one that does is a gap, not an expectation.
- **Baselines.** **A complete new baseline is required.** Every `dz_nvp > 0` entry produces comparable output for the first time. Generate as `<branchbasename>.<shorthash>`; compare against the previous baseline.

---

### Task 15: History snow-field fill + SNO_* slices

**In brief.** Task 15 keeps the moss slot out of the eight `levsno` history fields — `SNO_ABS`, `SNO_ABS_ICE`, `SNO_T`, `SNO_T_ICE`, `SNO_TK`, `SNO_TK_ICE`, `SNO_Z` and `SNO_Z_ICE`. The bottom-justification in `hist_set_snow_field_2d` ends at `jbot_sno` instead of 0, and `num_snow_layers` is already honest. The one open question is whether excluding it in the fill is sufficient or the field slices need their bounds changed too — decide by reading the fill rather than by inspecting the registrations.
...
- **Answer changes.** **History output changes on NVP columns**, deliberately: the eight `levsno` fields stop reporting the moss slot as the bottom snow layer and start reporting the true snow pack. Pre-existing tests are untouched — the fill keys on `col%jbot_sno(c)`, which is 0 everywhere with NVP off, and **no field's dimension changes**, since the registrations are not touched. That last point is what protects every existing baseline; if a `levsno` dimension moves, Step 1 was done wrong.
...
- **Baselines.** **A complete new baseline is required** — NVP-on history content changes. This holds only because the `clm/Nvp` testmod requests these fields (Task 7 Step 5b); all eight are `default='inactive'`, so without that line this task would change nothing observable in any test and there would be nothing to baseline.

---

### Task 16: Unit-test coverage audit

**In brief.** Task 16 no longer batches unit tests — Global Constraints now require each task to write and commit its tests *before* its implementation, at Step 0a, and to bring them to green without editing them. What is left here is the audit: review the coverage accumulated across Tasks 5-15 against the spec, name what is untested, and close the gaps. The one case that was still queued in this task — `CombineSnowLayers` on an NVP column where the bottom layer vanishes, depositing into the moss when it has thickness and passing through to soil when it does not, booking `qflx_sl_top_soil` either way — moves to the task that first makes it writable, which is the one that gives the moss water to hold.
...
- [ ] **Step 1 — audit.** Walk the spec's requirement list against the tests that now exist, and produce a written table: requirement, the test that pins it, and the mutation that proves the test has teeth. Any row backed by neither a red-first history nor a mutation is not covered, whatever the assertions look like. Close what is missing here. **Every test this task itself writes is necessarily green-throughout** — it covers code that already exists and already works, so there is no red to record — which means all of them owe mutation evidence, and the "cut it if no evidence can be produced" rule bites harder here than anywhere else.
...
- [ ] **Step 0a — write and commit this task's unit tests, before any of its implementation exists.** Dispatch the test-writing subagent (Execution Process step 3a). It writes the pFUnit coverage for what this task will land, **runs it**, and labels each test red-first or green-throughout: the green ones carry mutation evidence into the hand-off, the red ones carry the fact that they failed here. Standalone commit, never amended afterwards. The implementer that follows may not touch these files, and they must pass **unchanged** by the end of the task. If this task genuinely lands nothing a unit test can reach, say so explicitly and say why; silence is not a considered "none". This task removes traces and lands no new behaviour, so the expected answer is no new tests — say that in the hand-off rather than leaving it implicit.
...
- **Unit** — satisfied by a named pFUnit test, together with the evidence that it has teeth: for a red-first test, the Step 0a commit at which it failed; for a green-throughout one, the mutation. This is the only kind that can reach a requirement the suites are structurally blind to, which is why 10.4b is unit-only rather than unit-as-well.
