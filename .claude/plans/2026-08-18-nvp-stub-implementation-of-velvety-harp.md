...
5. Each task ends with: verification passes (below) → **one commit** for the task.
   - **Never `git commit -a` / `git commit -am`.** Stage explicitly — `git add <paths>` — and read `git status` first. A subagent's work often sits uncommitted in the tree while the orchestrator writes a plan note; `-a` sweeps it into that note's commit, under a message describing something else entirely. This happened at Task 5b: the whole `CombineSnowLayers` reindex landed in a commit titled "Record the ZeroEmptySnowLayers sequencing hazard in Task 5c", and the commit that claimed to do the reindex held only the review fixes. Both were reworded afterwards, but the defect survived two reviews and was caught only by the user reading the log — so the guard has to be at commit time, not at review time.
...
- [ ] **Step 0: Plan review (orchestrator; do not delegate).** Read this task's text against the spec and the code it touches. **STOP** and put to the user: clarifying questions, problems foreseen, cleanup the task text needs, unmet dependencies. Write the resolutions into this plan file before dispatching the implementer. Known going in: these tests arrive after the code they cover — confirm whether that ordering stands, or whether the remaining case should instead be written alongside Task 14 (superpowers:test-driven-development would put it first). The test-authoring rules that used to sit here are now in Global Constraints, so every task carries them.
...
- [ ] **Step 2a — close a known coverage gap: `DivideSnowLayers`' interface-recursion guard is untested.** Reverting `j <= col%get_jbot_snow(c)` in that routine's final recursion loop to the pre-NVP `j <= 0` leaves all 60 unit-test binaries passing — confirmed by running the mutation, not inferred. Nothing else in the suite exercises `DivideSnowLayers`, so that guard has *zero* coverage today. The cause is a degenerate fixture, in the sense of the Global Constraint above: `setupColumn` in `test_SnowHydrology_divideSnowLayers.pf` writes a moss geometry that is self-consistent under the very recursion the guard controls (`zi(c,0) = 0`, `dz(c,0) = dz_nvp`, `z(c,0) = -0.5*dz_nvp`, `zi(c,-1) = -dz_nvp`), so a pass at `j = 0` recomputes exactly what setup wrote. The fix is one line — give the moss node a depth that is deliberately *not* its layer midpoint, e.g. `col%z(c,0) = -0.25_r8*dz_nvp`, and say at the assignment why. Neither `DivideSnowLayers` nor `InitSnowLayers` ever reads `z(c,0)`; both only write it, so nothing else moves. It was left undone in the S3 review round as a deliberate user decision (the fixture becomes unphysical), with `assertMossUntouched`'s comment corrected to state the gap; revisit it here. Whatever is decided, the mutation above is the acceptance test.
