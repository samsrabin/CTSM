...
- `t_soisno(c,0)` (and the `t_nvp_col` mirror) on `nvp_is_empty` columns: **set post-solve to the tied/interface value** (recommended) rather than left as NaN with explicit handling at every access point. The trade-off, considered:
  ...
  - *Mitigation*: the zero-thickness test (§10) is the loud failure the NaN approach would have provided — any missed guard that matters shows up as a `use_nvp=T, dz_nvp=0` vs `use_nvp=F` difference — and it catches the cases NaN-trapping can't (a guard missing where the tie value is *wrong* but finite). So: tie value in the code, zero-thickness test as the tripwire.
...
5. **Zero-thickness invariant**: `use_nvp = .true.` with `dz_nvp = 0` must reproduce `use_nvp = .false.` answers **to roundoff, not exactly.** §3 keeps the flux-continuity moss row in the matrix — deleting it would make the snow↔soil coupling span 3 rows and break `nband = 5` — so the band solve carries one row more than the stock run, `dgbsv` takes a different pivot sequence, and the snow-bottom row's own coefficient equals stock's only after elimination. A bit-for-bit comparison is therefore not available, and neither is a `SystemTestsCompareTwo` test type in the `LWISO`/`LCISO` mould, since that framework passes only on exact agreement. What the case asserts is that the run completes with every balance check armed and closed; a `cprnc` against the `use_nvp = .false.` history is a diagnostic for gross divergence, never a PASS/FAIL gate. The four skip paths this was meant to cover — the §2 invariant, the §3 degenerate row, the §4 zero-dz routings and the §4b fraction rule — are pinned by unit tests instead.
...
7. **Sensitivity sanity — DROPPED.** ~~transmissivity = 1 + tiny `dz_nvp` ≈ stock answers (not exact — a real moss layer conducts — but bounded and converging as `dz_nvp → 0`, which the zero-thickness invariant anchors); conservation must hold exactly at any parameter values.~~ The bounded-convergence half has no automation path: `SystemTestsCompareTwo` passes only on exact agreement, with no tolerance anywhere in its cprnc path, and asserting convergence needs at least two parameter values, which a two-case framework does not provide. Its second clause — conservation exact at any parameter values — is not lost: §10.2 keeps every balance check armed and §10.6 requires exact closure at `frac_nvp` 0.3 and 0.7. **Consequence to carry:** §1.9's requirement that transmissivity = 1 reproduce stock energy deposition everywhere still stands as a design requirement, but no test now verifies it — it rests on review of the radiation task.
