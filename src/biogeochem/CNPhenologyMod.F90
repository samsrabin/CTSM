      ...
      if (do_plant_prescribed .and. (.not. generate_crop_gdds) .and. (.not. ignore_rx_crop_gdds) .and. crop_inst%rx_cultivar_gdds_thisyr(p,s) .gt. 0._r8) then
         ...
         if (ivt(p) == ntmp_soybean .or. ivt(p) == nirrig_tmp_soybean .or. &
            ...
            if (gddmaturity(p) < gddmin(ivt(p))) then
               write(iulog,*) 'srts: Some soy patch (ivt ',ivt(p),') has calculated gddmaturity ',gddmaturity(p),'; using gddmin(ivt(p)) instead (',gddmin(ivt(p)),')'
            endif
         end if
         if (ivt(p) == ntmp_corn .or. ivt(p) == nirrig_tmp_corn .or. &
               ivt(p) == ntrp_corn .or. ivt(p) == nirrig_trp_corn .or. &
            ...
            if (gddmaturity(p) < gddmin(ivt(p))) then
               write(iulog,*) 'srts: Some corn/sgc/misc/swg patch (ivt ',ivt(p),') has calculated gddmaturity ',gddmaturity(p),'; using gddmin(ivt(p)) instead (',gddmin(ivt(p)),')'
            endif
         end if
         if (ivt(p) == nswheat .or. ivt(p) == nirrig_swheat .or. &
            ...
            if (gddmaturity(p) < gddmin(ivt(p))) then
               write(iulog,*) 'srts: Some swheat/cotton/rice patch (ivt ',ivt(p),') has calculated gddmaturity ',gddmaturity(p),'; using gddmin(ivt(p)) instead (',gddmin(ivt(p)),')'
            endif
         end if
