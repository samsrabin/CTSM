...
contains
  ...
  subroutine CropPhenology(num_pcropp, filter_pcropp                     , &
    ...
    associate(                                                                   & 
      ...
      ! SSR troubleshooting
      verbose_londeg = 20._r8
      verbose_latdeg = 36.94_r8
...
!             .and. ivt(p) == verbose_ivt
         ...
         if (croplive(p) .and. idop(p) <= jday .and. sowing_count(p) == 0) then
             ...
             if ( verbose ) then
                ...
                write (iulog,'(a,i4,a,i4)') p_str,' cpv   sdates_thisyr(1) ',crop_inst%sdates_thisyr_patch(p,1),' -> ',idop(p)
             end if
            ...
            if (abs(offset_counter(p) - dt) <= dt/2._r8) then
               ...
               if (ivt(p) >= npcropmin) then
                  ...
                  do k = repr_grain_min, repr_grain_max
                     ...
                     if (reproductivec(p,k) + repr_grainc_to_food_thispool * dt .ne. 0) then
                         if (h .le. 0) then
