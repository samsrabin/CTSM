...
contains
  ...
  subroutine CNPhenologySetParams( )
    ...
    !
    params_inst%crit_dayl             = 50400._r8     ! Seconds WW changed to 14 hours
...
!KO    params_inst%ndays_on              = 15._r8        ! Days
...
! WW            crit_onset_gdd = exp(4.2_r8 + 0.13_r8*(annavg_t2m(p) - SHR_CONST_TKFRZ))
!KO
            crit_onset_gdd = crit_onset_gdd_sf(ivt(p)) * exp(4.2_r8 + 0.13_r8*(annavg_t2m(p) &
                             - SHR_CONST_TKFRZ))
...
!KO
  ...
  function SeasonalDecidOnset( onset_gdd, onset_gddflag, soilt, soila10, t_a5min, dayl, &
    ...
    !
    ! !LOCAL VARIABLES:
    ! WW modified this from 0.1 to 1.0, unused with change to onset_thresh_depends_on_veg = .false. 
    real(r8), parameter :: snow5d_thresh_for_onset      = 1.0_r8          ! 5-day snow depth threshold for leaf onset
...
! WW added so only collecting GDD when > 0 deg C
    if (onset_gddflag == 1.0_r8 .and. soilt > (SHR_CONST_TKFRZ+0._r8)) then
        onset_gdd = onset_gdd + (soilt-(SHR_CONST_TKFRZ+0._r8))*fracday
    end if
