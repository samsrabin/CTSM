    ...
    associate(                                      & 
      ...
      do fp = 1,num_soilp
         ...
         if(.not.  use_matrixcn)then
            ...
            ! displayed pools
            if (isnan(cf_veg%m_leafc_to_litter_patch(p))) then
               write(iulog,"(a,i3,a,a,a,i4)") "ssrts   NaN m_leafc_to_litter_patch p ",p," file ",__FILE__," line ",__LINE__
            endif
  ...
  subroutine CStateUpdate2h(num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    associate(                                     & 
      ...
      do fp = 1,num_soilp
         ...
         if(.not. use_matrixcn)then
            ! displayed pools
            if (isnan(cf_veg%hrv_leafc_to_litter_patch(p))) then
               write(iulog,"(a,i3,a,a,a,i4)") "ssrts   NaN hrv_leafc_to_litter_patch p ",p," file ",__FILE__," line ",__LINE__
            endif
            cs_veg%leafc_patch(p) = cs_veg%leafc_patch(p)                           &
  ...
  subroutine CStateUpdate2g(num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    associate(                                     & 
      ...
      do fp = 1,num_soilp
         ...
         ! displayed pools
         if (isnan(cf_veg%gru_leafc_to_litter_patch(p))) then
              write(iulog,"(a,i3,a,a,a,i4)") "ssrts   NaN gru_leafc_to_litter_patch p ",p," file ",__FILE__," line ",__LINE__
         endif
         cs_veg%leafc_patch(p) = cs_veg%leafc_patch(p)                           &
