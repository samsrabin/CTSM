...
contains
  ...
  subroutine CNFireArea (this, bounds, num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    use pftconMod            , only: nc4_grass, nc3crop
    ...
    associate(                                                                      &
     ...
     do fp = 1,num_soilp
      ...
      do fp = 1, num_soilp
        ...
        ! For crop
        if( forc_t(c)  >=  SHR_CONST_TKFRZ .and. patch%itype(p) >  nc4_grass  &
           .and. kmo == abm_lf(c) .and. burndate(p) >= 999 &
           .and. patch%wtcol(p) > 0._r8)then ! catch  crop burn time
