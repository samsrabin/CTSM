...
!==============================================================================
  ...
  subroutine get_apply_tillage_multipliers(idop, num_soilp, filter_soilp, c, decomp_k, i_act_som, i_slo_som, i_pas_som, i_cel_lit, i_lig_lit)
    ...
    do j = 1,5
        ...
        if (tillage_mults(i_cel_lit) > 4._r8) then
            write(iulog,'(a,i2,a)') 'Big tillage_mults i_cel_lit (',i_cel_lit,')'
            write(iulog,*) '   tillage_mults ',tillage_mults(i_cel_lit)
        end if
        if (tillage_mults(i_lig_lit) > 4._r8) then
            write(iulog,'(a,i2,a)') 'Big tillage_mults i_lig_lit (',i_lig_lit,')'
            write(iulog,*) '   tillage_mults ',tillage_mults(i_lig_lit)
        end if
        if (tillage_mults(i_act_som) > 4._r8) then
            write(iulog,'(a,i2,a)') 'Big tillage_mults i_act_som (',i_act_som,')'
            write(iulog,*) '   tillage_mults ',tillage_mults(i_act_som)
        end if
        if (tillage_mults(i_slo_som) > 4._r8) then
            write(iulog,'(a,i2,a)') 'Big tillage_mults i_slo_som (',i_slo_som,')'
            write(iulog,*) '   tillage_mults ',tillage_mults(i_slo_som)
        end if
        if (tillage_mults(i_pas_som) > 4._r8) then
            write(iulog,'(a,i2,a)') 'Big tillage_mults i_pas_som (',i_pas_som,')'
            write(iulog,*) '   tillage_mults ',tillage_mults(i_pas_som)
        end if
    end do
