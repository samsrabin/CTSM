...
BASELINE_VERSION_OF_SCRIPT_INPUT_FILES = "improve-rxcropmaturity-tests.3e6bf134"
...
class RXCROPMATURITYSHARED(SystemTestsCommon):
    ...
    def __init__(self, case: Case = None) -> None:
        ...
        self._caseroot_rxboth: str = self._case.get_value("CASEROOT")
        self._caseroot_gddgen: str = f"{self._caseroot_rxboth}.gddgen"

        # Cases
        self._case_rxboth: Case = self._case
        self._case_gddgen: Case = None
        self._create_case_gddgen()
    ...
    def setup_phase(self, *args, **kwargs) -> None:
        ...
        os.chdir(self._caseroot_rxboth)
    ...
    def run_phase(self) -> None:
        ...
        # -------------------------------------------------------------------
        # (1) Perform GDD-generating run and generate prescribed GDDs file
        # -------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------
        # (2) Perform Prescribed Calendars run
        # -------------------------------------------------------------------
        os.chdir(self._caseroot_rxboth)
        self._set_active_case(self._case_rxboth)
        self.run_indv()
        ...
        # -------------------------------------------------------------------
        # (3) Check Prescribed Calendars run
        # -------------------------------------------------------------------
    ...
    def _get_dirs_for_scripts(self):
        if self._scriptsonly_test:
            baseline_dir = self._get_baseline_dir(self._case_gddgen)
            lnd_grid = self._case_gddgen.get_value("LND_GRID")
    ...
    def _setup_case_gddgen(self):
        ...
        self._get_flanduse_timeseries_in(self._case_gddgen)
        if self._flanduse_timeseries_in is not None:
            # Download files from the server, if needed
            self._case_gddgen.check_all_input_data()

            # Copy needed file from original to gddgen directory
            shutil.copyfile(
                os.path.join(self._caseroot_rxboth, ".env_mach_specific.sh"),
                os.path.join(self._caseroot_gddgen, ".env_mach_specific.sh"),
            )
    ...
    def _create_case_gddgen(self):
        """
        Create the GDD-Generating case, if needed. If it already exists, just use that.
        """
        if os.path.exists(self._caseroot_gddgen):
            self._case_gddgen = Case(self._caseroot_gddgen)
        else:
            log(logger, "RXCROPMATURITY log:  cloning")
            self._case_gddgen = self._case.create_clone(
                self._caseroot_gddgen, keepexe=True
            )
    ...
    def _run_fsurdat_modifier(self) -> None:
        ...
        self._fsurdat_out = os.path.join(
            self._caseroot_gddgen, f"{filename_in_noext}.all_crops_everywhere{ext}"
        )
    ...
    def _run_generate_gdds(self, input_dir: str):
        self._generate_gdds_outdir = os.path.join(
            self._caseroot_gddgen, "generate_gdds_out"
        )
    ...
    def _get_flanduse_timeseries_in(self, case: Case) -> None:
        ...
        self._lnd_in_path = os.path.join(self._caseroot_gddgen, "CaseDocs", "lnd_in")
